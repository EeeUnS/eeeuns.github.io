...
enum을 string으로 바꿀때 아래 [magic_enum](https://github.com/Neargye/magic_enum) 라이브러리를 쓸 수도 있지만, 사실 위의 XMacro 패턴으로 커버가 가능하다.대해서 제대로 스펙을 파악하고 진행하는데 어려움이 많을 수 밖에없다. 그런 부분에서 이런 놓치는 부분을 작업단에서 잡아줄 수 있다.
내부 방식이 복잡한 템플릿 구조는 컴파일 속도에 악영향이 간다.
...
결론은 compile time에 놓침 없이 잡아주는게 가장 큰 장점이다.
