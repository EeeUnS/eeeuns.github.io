...
$\{a_0,a_2, ... , a_n \}$으로 나타내도 $A(x)$를 확정지을 수
...
집합이$\{\omega_n^1, \omega_n^2 , ... , \omega_n^{n-1}\}$인
$x$좌표에 대한 $y$값의 집합이다.
...
계수 벡터 $A = \{a_0, a_1, a_2, a_3 \}$인 경우를 생각해보자.
이를 점값표현으로 변환, 역변환해 본다.

1.  분할: $\{a_0, a_2 \}$, $\{a_1, a_3\}$

2.  분할:
    $\{a_0\}, \{a_2\}, \{a_1\}, \{a_3\}$

3.  결합:
    $\{a_0 + a_2, a_0 - a_2 \}\{a_1 + a_3, a_1 - a_3\}$

4.  결합:
    $\{(a_0 + a_1 + a_2 + a_3), (a_0 + \omega a_1 - a_2 -\omega a_3) , (a_0 - a_1 + a_2 - a_3), (a_0 - \omega a_1 - a_2 + \omega a_3) \}$
...
1.  분할: $\{y_0, y_2 \}$, $\{y_1, y_3\}$

2.  분할:
    $\{y_0\}, \{y_2\}, \{y_1\}, \{y_3\}$

3.  결합:
    $\{y_0 + y_2, y_0 - y_2 \}\{y_1 + y_3, y_1 - y_3\}$

4.  결합:
    $\{(y_0 +y_1 + y_2 + y_3), (y_0 - \omega y_1 + y_2 +\omega y_3) , \\(y_0 - y_1 +y_2 - y_3), (y_0 + \omega y_1 - y_2 - \omega y_3) \}$
